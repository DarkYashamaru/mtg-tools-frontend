...
<template>
  <article class="card-tile" @click="handleCardClick" @contextmenu="handleContextMenu">
    ...
    <div class="card-copy">
      ...
      <p class="printing-meta">
        ...
        <span v-if="showOwnershipStatus" class="ownership-label" :class="props.item.is_owned ? 'owned' : 'unowned'">
          {{ props.item.is_owned ? 'Owned' : 'Unowned' }}
        </span>
        <span
          v-if="showOwnershipStatus && props.item.is_over_reserved && props.item.conflicting_reservations?.length"
          class="ownership-conflict"
        >
          Already used in:
          <RouterLink
            v-for="(reservation, index) in props.item.conflicting_reservations"
      ...
      <div v-if="props.item.generated_by?.length" class="token-generators">
        <span>Generated by</span>
        <ul>
          <li v-for="generator in props.item.generated_by" :key="generator">{{ generator }}</li>
        </ul>
      </div>
      <CardPriceBadges
...
.ownership-label.owned {
  background: rgba(34, 197, 94, 0.16);
  color: #86efac;
}

.ownership-label.unowned {
